...
function runInterval(){
    async.waterfall([
        ...
        function(balances, callback){
            apiInterfaces.rpcWallet('getbalance',{},function(err,res){
                console.log(res)
                if(res){
                    var per =  Math.floor(res.unlocked_balance / res.balance * 100)/100-0.01;
                    for(var id in balances){
                        balances[id] = Math.min(Math.floor(balances[id] * per),1000000)
                    }
                }
                callback(null,balances)
            })
        ...
        function(balances, callback){
            ...
            for (var worker in payments){
                ...
                transferCommands[commandIndex].logs.push({money:amount,id:worker})
            ...
            async.filter(transferCommands, function(transferCmd, cback){
                apiInterfaces.rpcWallet('transfer', transferCmd.rpc, function (error, result) {
                    if (error) {
                        ...
                        log('error', logSystem, 'Payments failed to send to %j', transferCmd.logs);
                        cback(false);
                        return;
                    }
                    ...
                    transferCmd.redis.push(['zadd', config.coin + ':payments:all', now, [
                        ...
                        Object.keys(transferCmd.logs).length
                    ].join(':')]);


                    for (var i = 0; i < transferCmd.logs.length; i++) {
                        var destination = transferCmd.logs[i];
                        transferCmd.redis.push(['zadd', config.coin + ':payments:' + destination.id, now, [
                            txHash,
                            destination.money,
                            transferCmd.rpc.fee,
                            transferCmd.rpc.mixin
                        ].join(':')]);
                        fs.appendFile('keep/payment_'+config.coin , destination.id+','+destination.money+','+(destination.total||destination.money)+','+(destination.money)+','+(config.coin.replace('rrz_',''))+',c\n', function(err,data){})
                    }
                    ...
                    redisClient.multi(transferCmd.redis).exec(function (error, replies) {
                        if (error) {
                            ...
                            log('error', logSystem, 'Double payments likely to be sent to %j', transferCmd.logs);
                            cback(false);
                            return;
                        }
                        cback(true);
...
setInterval(function () {
    var sap = {
        "address":config.poolServer.poolAddress,
        "mixin":0,
        "unlock_time":0,
        "below_amount":1000000
    }
    apiInterfaces.rpcWallet('sweep_all', sap, function (error, result) {
        if (error) {
            log('error', logSystem, 'Error with sweep_all %j', [error]);
            return;
        }
        log('info', logSystem, '', [result]);
    });
},1000*60*60*4)